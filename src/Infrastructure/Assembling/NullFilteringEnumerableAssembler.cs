...
namespace VP.FF.PT.Common.Infrastructure.Assembling
{
    ...
    /// </summary>
    /// <remarks>
    /// Will filter out null values from the original enumerable.
    /// </remarks>
    [Export(typeof(NullFilteringEnumerableAssembler<,>))]
    public class NullFilteringEnumerableAssembler<TFrom, TTo> : EnumerableAssembler<TFrom, TTo>
        where TFrom : class 
        where TTo :class
    {
        ...
        /// <param name="assembleConfig">The assemble config for the instance assembler.</param>
        /// <returns>The resulting enumerable.</returns>
        public override IEnumerable<TTo> Assemble(IEnumerable<TFrom> fromItems, dynamic assembleConfig = null)
        {
            if (fromItems == null)
                yield break;
            foreach (TFrom fromItem in fromItems)
                if (fromItem != null)
                    yield return _instanceAssembler.Assemble(fromItem, assembleConfig);
        }
    }
}
